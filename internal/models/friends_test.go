...
func testFriendToOneUserUsingFriend(t *testing.T) {
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	if err = local.L.LoadFriend(ctx, tx, false, (*[]*Friend)(&slice), nil); err != nil {
		t.Fatal(err)
	}
...
func testFriendToOneUserUsingUser(t *testing.T) {
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	slice := FriendSlice{&local}
	if err = local.L.LoadUser(ctx, tx, false, (*[]*Friend)(&slice), nil); err != nil {
		t.Fatal(err)
	}
	if local.R.User == nil {
		t.Error("struct should have been eager loaded")
...
func testFriendToOneSetOpUserUsingFriend(t *testing.T) {
	var err error
...
func testFriendToOneSetOpUserUsingUser(t *testing.T) {
	var err error
...
var (
	friendDBTypes = map[string]string{`ID`: `integer`, `UserID`: `integer`, `FriendID`: `integer`}
	_             = bytes.MinRead
)

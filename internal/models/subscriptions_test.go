...
func testSubscriptionToOneUserUsingSubscriptionRequestor(t *testing.T) {
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	local.SubscriptionRequestorID = foreign.ID
	if err := local.Insert(ctx, tx, boil.Infer()); err != nil {
		t.Fatal(err)
	}

	check, err := local.SubscriptionRequestor().One(ctx, tx)
	if err != nil {
		t.Fatal(err)
	}
	...
	if err = local.L.LoadSubscriptionRequestor(ctx, tx, false, (*[]*Subscription)(&slice), nil); err != nil {
		t.Fatal(err)
	}
	if local.R.SubscriptionRequestor == nil {
		t.Error("struct should have been eager loaded")
	}

	local.R.SubscriptionRequestor = nil
	if err = local.L.LoadSubscriptionRequestor(ctx, tx, true, &local, nil); err != nil {
		t.Fatal(err)
	}
	if local.R.SubscriptionRequestor == nil {
		t.Error("struct should have been eager loaded")
...
func testSubscriptionToOneUserUsingSubscriptionTarget(t *testing.T) {
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	local.SubscriptionTargetID = foreign.ID
	if err := local.Insert(ctx, tx, boil.Infer()); err != nil {
		t.Fatal(err)
	}

	check, err := local.SubscriptionTarget().One(ctx, tx)
	if err != nil {
		t.Fatal(err)
	}
	...
	slice := SubscriptionSlice{&local}
	if err = local.L.LoadSubscriptionTarget(ctx, tx, false, (*[]*Subscription)(&slice), nil); err != nil {
		t.Fatal(err)
	}
	if local.R.SubscriptionTarget == nil {
		t.Error("struct should have been eager loaded")
	}

	local.R.SubscriptionTarget = nil
	if err = local.L.LoadSubscriptionTarget(ctx, tx, true, &local, nil); err != nil {
		t.Fatal(err)
	}
	if local.R.SubscriptionTarget == nil {
		t.Error("struct should have been eager loaded")
...
func testSubscriptionToOneSetOpUserUsingSubscriptionRequestor(t *testing.T) {
	var err error
	...
	for i, x := range []*User{&b, &c} {
		err = a.SetSubscriptionRequestor(ctx, tx, i != 0, x)
		if err != nil {
			t.Fatal(err)
		}

		if a.R.SubscriptionRequestor != x {
			t.Error("relationship struct not set to correct value")
		}

		if x.R.SubscriptionRequestorSubscriptions[0] != &a {
			t.Error("failed to append to foreign relationship struct")
		}
		if a.SubscriptionRequestorID != x.ID {
			t.Error("foreign key was wrong value", a.SubscriptionRequestorID)
		}

		zero := reflect.Zero(reflect.TypeOf(a.SubscriptionRequestorID))
		reflect.Indirect(reflect.ValueOf(&a.SubscriptionRequestorID)).Set(zero)
		...
		if a.SubscriptionRequestorID != x.ID {
			t.Error("foreign key was wrong value", a.SubscriptionRequestorID, x.ID)
		}
	}
}
func testSubscriptionToOneSetOpUserUsingSubscriptionTarget(t *testing.T) {
	var err error
	...
	for i, x := range []*User{&b, &c} {
		err = a.SetSubscriptionTarget(ctx, tx, i != 0, x)
		if err != nil {
			t.Fatal(err)
		}

		if a.R.SubscriptionTarget != x {
			t.Error("relationship struct not set to correct value")
		}

		if x.R.SubscriptionTargetSubscriptions[0] != &a {
			t.Error("failed to append to foreign relationship struct")
		}
		if a.SubscriptionTargetID != x.ID {
			t.Error("foreign key was wrong value", a.SubscriptionTargetID)
		}

		zero := reflect.Zero(reflect.TypeOf(a.SubscriptionTargetID))
		reflect.Indirect(reflect.ValueOf(&a.SubscriptionTargetID)).Set(zero)
		...
		if a.SubscriptionTargetID != x.ID {
			t.Error("foreign key was wrong value", a.SubscriptionTargetID, x.ID)
		}
	}
}
...
var (
	subscriptionDBTypes = map[string]string{`ID`: `integer`, `SubscriptionRequestorID`: `integer`, `SubscriptionTargetID`: `integer`}
	_                   = bytes.MinRead
)
